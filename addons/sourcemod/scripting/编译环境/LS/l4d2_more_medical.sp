...
{
	g_hMoreMedical = CreateConVar("l4d2_more_medical", "7", "把需要启用的功能数字相加. 0=禁用, 1=声音, 2=倍数, 4=Medical.", CVAR_FLAGS);
...
void IsUpdateEntCount(int client, int g_Multiple, bool g_bPrompt, bool g_bContent, bool g_bMoreCheck, int g_iClientNumber, int g_iSurvivorLimit)
{
	...
	if(g_iMoreMedical & SurvivorsPrompt && g_bMoreCheck)
	{
		if(g_bPrompt)
			if(!g_bPlayerPrompt)
				PrintToChatAll("\x04[Medical]\x03%N\x05%s\x04(\x03%i\x05/\x03%d\x04)\x03...\x04%s", client, g_bContent ? "正在连接" : "断开连接", g_iPlayerNumber, g_iSurvivorLimit, g_bContent ? "(づˉ*ˉ)づ" : "╮(๑•́ ₃•̀๑)╭");
			else
				PrintToChatAll("\x04[Medical]\x03%N\x05%s\x04(\x03%i\x05/\x03%d\x04)\x03,\x05设置为\x03%s\x05倍医疗备品.", client, g_bContent ? "正在连接" : "断开连接", g_iPlayerNumber, g_iSurvivorLimit, g_sMedical);
		else
			PrintToChatAll("\x04[Medical]\x05当前人数为\x03:\x04(\x03%i\x05/\x03%d\x04)\x03,\x05设置为\x03%s\x05倍医疗备品.", g_iClientNumber, g_iSurvivorLimit, g_sMedical);
	}
}
