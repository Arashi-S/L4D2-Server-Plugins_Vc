...
public void OnPluginStart() {
	...
	g_cSpawnSize = 					CreateConVar("ss_spawn_size",			"1",						"一次产生多少只特感", _, true, 1.0, true, 32.0);
	...
	g_cSpawnTimeMin =				CreateConVar("ss_time_min",				"30.0",						"特感的最小产生时间", _, true, 0.1);
	...
	g_cBaseLimit =					CreateConVar("ss_base_limit",			"2",						"生还者团队不超过4人时有多少个特感", _, true, 0.0, true, 32.0);
	...
	g_cSpawnRangeMin =				CreateConVar("ss_spawnrange_min",		"250.0",					"特感最小生成距离", _, true, 0.0);
	g_cSpawnRangeMax =				CreateConVar("ss_spawnrange_max",		"1500.0",					"特感最大生成距离", _, true, 0.0);

	g_cFirstSpawnTime = 			CreateConVar("ss_first_time",			"16.0",						"玩家离开安全区域后第一波特感的刷新时间", _, true, 0.0);
