			...
			switch (GetClientTeam(client))
			{
				case 1:
				{
					...
					if (iBot != 0)
					{
						...
						{
							case 1:
								PrintToChat(client, "\x04[HspAF]\x05倒地或挂边输入指令\x03!zs\x05或\x03!kill\x05或\x04(\x03自杀\x04)\x05可自杀.");//聊天窗提示.
							case 2:
								PrintToChat(client, "\x04[HspAF]\x05输入指令\x03!zs\x05或\x03!kill\x05或\x03自杀\x05可自杀.");//聊天窗提示.
							default:
								PrintToChat(client, "\x04[HspAF]\x05输入指令\x03!zs\x05或\x03!kill\x05或\x03自杀\x05可自杀.");//聊天窗提示.
						}
					}
				...
				case 3:
				{
					...
					{
						case 1:
							PrintToChat(client, "\x04[HspAF]\x05非灵魂状态输入指令\x03!zs\x05或\x03!kill\x05或\x04(\x03自杀\x04)\x05可自杀.");//聊天窗提示.
						case 2:
							PrintToChat(client, "\x04[HspAF]\x05输入指令\x03!zs\x05或\x03!kill\x05或\x03自杀\x05可自杀.");//聊天窗提示.
						default:
							PrintToChat(client, "\x04[HspAF]\x05输入指令\x03!zs\x05或\x03!kill\x05或\x03自杀\x05可自杀.");//聊天窗提示.
			...
			switch (GetClientTeam(client))
			{
				case 1:
				{
					...
					if (iBot != 0)
						if(g_iSurvivor > 0)
							...
							PrintToChat(client, "\x04[HspAF]\x05生还者自杀指令未启用.");
					else
						PrintToChat(client, "\x04[HspAF]\x05旁观者你自杀个寄吧.");
				}
				case 2:
				{
					if(g_iSurvivor > 0)
				...
				case 3:
				{
					if(g_iInfected > 0)
						...
						PrintToChat(client, "\x04[HspAF]\x05感染者自杀指令未启用.");
				}
			}
		}
	}
...
void IsSurvivorSuicide(int client, int victim, char[] sName, char[] sTeam)
{
	if (IsPlayerAlive(client))
	{
		switch (g_iSurvivor)
		{
			case 1:
			{
				if (!IsPlayerState(client))
				{
					...
					PrintToChatAll("\x04[HspAF]\x05(\x04%s\x05)\x03%s\x05呼吸过猛把自己呛死了.", sTeam, sName);
				}
				else
					PrintToChat(victim, "\x04[HspAF]\x05该指令只限倒地或挂边的%s使用.", sTeam);
			}
			case 2:
			{
				if(g_bPointsFall)
		...
		PrintToChat(victim, "\x04[HspAF]\x05尸体不能死第二次，但你可以吃屎.");
}
//感染者.
		...
		switch (g_iInfected)
		{
			case 1:
			{
				if (!GetEntProp(client, Prop_Send, "m_isGhost"))
				{
					...
					PrintToChat(victim, "\x04[HspAF]\x05灵魂状态禁止使用该指令.");
			}
			case 2:
			{
				ForcePlayerSuicide(client);//幸存者自杀代码.
