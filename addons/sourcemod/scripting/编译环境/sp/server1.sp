...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_team2", Cmd_AFKTurnClientToSurvivor, "Turn player to survivor");
...
public void OnClientConnected(int client)
{
    ...
    C_PrintToChatAll(messages[Msg_Connected], client);
...
public void OnClientDisconnect(int client)
{
    ...
    if (IsClientInGame(client))
        C_PrintToChatAll(messages[Msg_DisConnected], client);
...
public Action L4D_OnFirstSurvivorLeftSafeArea(int client)
{
    ...
    C_PrintToChatAll(messages[Msg_Warn]);
}
...
public Action Command_Jointeam(int client, const char[] command, int args)
{
    ...
    if (StrEqual(arg, "infected", true) || StrEqual(arg, "3", true))
    {
        if (IsInfected(client))
        {
            C_PrintToChat(client, messages[Msg_AlreadyInfected]);
            return Plugin_Handled;
        }

        if (!IsInfected(client) && !canInfectedJoin)
        {
            C_PrintToChat(client, messages[Msg_InfectedDisabled]);
            return Plugin_Handled;
        }

        if (isRoundStart && IsSurvivor(client))
        {
            C_PrintToChat(client, messages[Msg_RoundStart]);
            return Plugin_Handled;
        }

        C_PrintToChatAll(messages[Msg_JoinInfected], client);
        C_PrintToChat(client, messages[Msg_JoinSINotice]);
    }
...
public Action Event_WitchKilled(Event event, const char[] name, bool dontBroadcast)
{
        ...
        IsSurvivor(client) && IsPlayerAlive(client) && !IsPlayerIncap(client))
    {
        ...
        int targetHealth = GetPlayerHealth(client) + 30;
...
public Action Event_PlayerReplaceTankBot(Event event, const char[] name, bool dont_broadcast)
{
    ...
    if (GetInfectedClass(client) == view_as<int>(ZC_Tank))
    {
        ...
        C_PrintToChatAll(messages[Msg_TakeTank], tankPlayer);
...
public Action Event_CreateClipMenu(int client)
{
    ...
    SetMenuTitle(menu, "你是否要飞行复活？:");
    AddMenuItem(menu, "NOCLIP",   "起飞！");
    AddMenuItem(menu, "WLAK",   "灵魂跑飞不更香吗？");
    SetMenuPagination(menu, MENU_NO_PAGINATION);
    SetMenuExitButton(menu, true);
    DisplayMenu(menu, client, MENU_DISPLAY_TIME);
...
public Action Timer_ReStartMap(Handle timer, int client)
{
    if (GetSurvivorCount() > serverMaxSurvivorCount)
    {
        C_PrintToChatAll(messages[Msg_Error]);
...
public Action Timer_CheckPlayerTeam(Handle timer, int client)
{
    ...
    C_PrintToChat(client, messages[Msg_Notice]);
}
...
public Action Timer_PlayerTakeGhost(Handle timer, int client)
{
    ...
    ChangeClientTeam(client, 1);
    C_PrintToChat(client, messages[Msg_TakeGhost]);
    CloseGhostHandle(client);
}
...
public Action Timer_CheckTankFrustration(Handle timer, int tankClient)
{
    ...
    if (GetTankFrustration(tankClient) <= 5 && L4D2Direct_GetTankPassedCount() <= 1)
    {
        ...
        C_PrintToChatAll("{red}<{olive}Tank Rage{red}> {olive}Tank Rage Meter {red}Refilled!");
    }
    else if (L4D2Direct_GetTankPassedCount() >= 2 && GetTankFrustration(tankClient) == 5)
    {
        C_PrintToChatAll(messages[Msg_PlayerTankAI]);
...
public int Handle_ExecSpecMenu(Menu menu, MenuAction action, int client, int item)
{
    ...
    C_PrintToChatAll(messages[Msg_ForceSpectated], target);
...
public Action Cmd_PlayTank(int client, any args)
{
    ...
    {
        if (tankPlayer != -1)
            C_PrintToChat(client, messages[Msg_TankBeenTaken], tankPlayer);
        else if (L4D2_IsTankInPlay())
            C_PrintToChat(client, messages[Msg_TankAI]);
        else
        {
            tankPlayer = client;
            C_PrintToChatAll(messages[Msg_WillTakeTank], tankPlayer);
        }
    }
    else C_PrintToChat(client, messages[Msg_CantTakeTank]);
}
...
public Action Cmd_ChangeClass(int client, any args)
{
    ...
    if (!IsSpectator(client))
        ...
        C_PrintToChat(client, messages[Msg_UnableToUse]);
}
...
public Action Cmd_AFKTurnClientToInfected(int client, any args)
{
    ...
    else C_PrintToChat(client, messages[Msg_RoundStart]);
}
