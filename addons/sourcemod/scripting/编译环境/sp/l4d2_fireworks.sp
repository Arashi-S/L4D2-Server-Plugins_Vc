...
{
	g_hCvarAllow =		CreateConVar(	"l4d2_fireworks_allow",				"1",			"0=关闭插件，1=打开插件", CVAR_FLAGS);
	g_hCvarGas =		CreateConVar(	"l4d2_fireworks_allow_gas",			"0",			"允许油桶燃烧显示烟花（只对未被拾取的油桶有效）", CVAR_FLAGS);
	g_hCvarOxy =		CreateConVar(	"l4d2_fireworks_allow_oxygen",		"0",			"允许氧气罐爆炸显示烟花", CVAR_FLAGS);
	g_hCvarPro =		CreateConVar(	"l4d2_fireworks_allow_propane",		"0",			"允许煤气罐爆炸来燃放烟花", CVAR_FLAGS);
	g_hCvarChase =		CreateConVar(	"l4d2_fireworks_chase",				"10",			"0=关闭。僵尸被烟花爆炸吸引多长时间", CVAR_FLAGS, true, 0.0, true, 20.0);
	g_hCvarConvert1 =	CreateConVar(	"l4d2_fireworks_convert_gas",		"0",			"油桶有多少概率变成烟花盒", CVAR_FLAGS, true, 0.0, true, 100.0);
	g_hCvarConvert2 =	CreateConVar(	"l4d2_fireworks_convert_oxygen",	"0",			"氧气瓶多少概率变成烟花盒", CVAR_FLAGS, true, 0.0, true, 100.0);
	g_hCvarConvert3 =	CreateConVar(	"l4d2_fireworks_convert_propane",	"50",			"煤气罐有多少概率变成烟花盒", CVAR_FLAGS, true, 0.0, true, 100.0);
	g_hCvarDelayMax =	CreateConVar(	"l4d2_fireworks_delay_max",			"10",			"烟花最晚多长时间出现 (0=立刻出现).", CVAR_FLAGS, true, 0.0, true, 20.0);
	g_hCvarDelayMin =	CreateConVar(	"l4d2_fireworks_delay_min",			"3",			"烟花最早多长时间出现", CVAR_FLAGS, true, 0.0, true, 10.0);
	g_hCvarDelayRan =	CreateConVar(	"l4d2_fireworks_delay_ran",			"1",			"随机设置延迟烟花的数量。0=最大，1=随机", CVAR_FLAGS, true, 0.0, true, 1.0);
	g_hCvarMaxTime =	CreateConVar(	"l4d2_fireworks_delay_time_max",	"10.0",			"爆炸后产生延迟烟花的最大时间", CVAR_FLAGS, true, 2.0, true, 20.0);
	g_hCvarMinTime =	CreateConVar(	"l4d2_fireworks_delay_time_min",	"0.2",			"爆炸后延迟烟花出现的最小时间", CVAR_FLAGS, true, 0.1, true, 15.0);
	g_hCvarInitMax =	CreateConVar(	"l4d2_fireworks_init_max",			"3",			"初次爆炸时显示的最大烟花（0禁用）", CVAR_FLAGS, true, 0.0, true, 10.0);
	g_hCvarInitMin =	CreateConVar(	"l4d2_fireworks_init_min",			"0",			"初次爆炸时显示的最小烟花", CVAR_FLAGS, true, 0.0, true, 10.0);
	g_hCvarModes =		CreateConVar(	"l4d2_fireworks_modes",				"",				"在这些游戏模式下打开插件，用逗号分隔（没有空格）（空=全部）", CVAR_FLAGS );
	g_hCvarModesOff =	CreateConVar(	"l4d2_fireworks_modes_off",			"",				"在这些游戏模式下关闭插件，用逗号分隔（没有空格）（空=无）", CVAR_FLAGS );
	g_hCvarModesTog =	CreateConVar(	"l4d2_fireworks_modes_tog",			"0",			"在这些游戏模式中打开插件。0=全部，1=战役，2=生还者，4=对抗，8=清道夫。将数字相加", CVAR_FLAGS );
	g_hCvarType =		CreateConVar(	"l4d2_fireworks_type",				"15",			"要显示哪些烟花。数字相加。1=红色; 2=蓝色; 4=金色; 8=闪光", CVAR_FLAGS);
