...
public Action:Command_ChooseVote(iClient, iArgs)
{
	...
	if(IsVoteInProgress())
	{
		MC_ReplyToCommand(iClient, "[SM] %t", "Vote in Progress");
		return Plugin_Handled;
	}
...
public Menu_PlayersVote(iVote, iVoter)
{
	if(IsVoteInProgress())
	{
		MC_PrintToChat(iVoter, "[SM] %t", "Vote in Progress");
		return;
	}

	if(g_strVoteOverride[iVote][0] && !CheckCommandAccess(iVoter, g_strVoteOverride[iVote], 0))
	{
		MC_PrintToChat(iVoter, "[SM] %t", "No Access");
		return;
	}
	...
	{
		MC_PrintToChat(iVoter, "%t", "No Votes Remaining");
		return;
	}

	if(g_iVotePasses[iVote] >= g_iVoteMaxPasses[iVote] && g_iVoteMaxPasses[iVote] > 0)
	{
		MC_PrintToChat(iVoter, "%t", "Voting No Longer Available");
		return;
	}

	if(g_iMapTime < g_iVoteDelay[iVote])
	{
		MC_PrintToChat(iVoter, "%t", "Vote Delay", g_iVoteDelay[iVote] - g_iMapTime);
		return;
	}
	...
	{
		MC_PrintToChat(iVoter, "%t", "Vote Cooldown", g_iVoteCooldown[iVote] - (iTime - g_iVoteLast[iVoter][iVote]));
		return;
	}
	...
	if(iCount <= 0)
	{
		MC_PrintToChat(iVoter, "%t", "No Valid Clients");
		return;
	}
...
public MenuHandler_PlayersVote(Handle:hMenu, MenuAction:iAction, iVoter, iParam2)
{
	...
	if(iAction == MenuAction_Select)
	{
		...
		{
			MC_PrintToChat(iVoter, "%t", "Player no longer available");
			Menu_ChooseVote(iVoter);
			return;
		}
		...
		if(g_iVoteMaxCalls[iVote] > 0 && !CheckCommandAccess(iVoter, "customvotes_maxvotes", ADMFLAG_GENERIC))
		{
			...
			MC_PrintToChat(iVoter, "%t", "Votes Remaining", g_iVoteRemaining[iVoter][iVote]);
		}
		...
		if(g_strVoteCallNotify[iVote][0])
		{
			...
			MC_PrintToChatAll("%s", strNotification);
		}
...
public Vote_Players(iVote, iVoter, iTarget)
{
	...
	{
		MC_PrintToChat(iVoter, "%t", "Not Enough Valid Clients");
		return;
	}
...
public MenuHandler_MapVote(Handle:hMenu, MenuAction:iAction, iVoter, iParam2)
{
	...
	if(iAction == MenuAction_Select)
	{
		...
		{
			MC_PrintToChat(iVoter, "%t", "Already Voted");
			Menu_ChooseVote(iVoter);
			return;
		}
