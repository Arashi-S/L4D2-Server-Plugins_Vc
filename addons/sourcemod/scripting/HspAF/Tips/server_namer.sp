...
public void OnPluginStart()
{
	...
	g_hRefreshTime = CreateConVar("sn_refresh_time", "4", "服名的刷新时间，单位：秒", CVAR_FLAG, true, 0.1);
	g_hBaseServerName = CreateConVar("sn_base_server_name", "", "基本服名，配置则使用当前服名，未配置则使用文件中的服名", CVAR_FLAG, true, 0.0);
	g_hBaseModeName = CreateConVar("sn_base_mode_name", "普通战役", "基本模式名称", CVAR_FLAG, true, 0.0);
	// 获取其他 Cvar
	g_hHostName = FindConVar("hostname");
	g_hInfectedLimit = FindConVar("ss_si_limit");
	g_hSpawnInterval = FindConVar("ss_time_max");
	// AddChangeHook
