...
void OnPlayerHurt(Event event, const char[] name, bool dontBroadcast)
{
		...
		zombie_class == CHARGER) && damage >= GetConVarInt(cv_Damage))
	{
		...
		CPrintToChatAll("[{olive}伤害报告{default}] {red}%N{default}({green}%s{default}) 还剩下 {green}%d{default} 血! 你造成了 {green}%2.1f{default} 点伤害!", attacker, IsFakeClient(attacker)?"AI":SI_Names[zombie_class], remaining_health, GetConVarFloat(cv_Damage));
